...
class BreakoutStrategy:
    """
    Implements a simple breakout trading strategy.
    """
    def __init__(self, symbol: str, lookback: int = 20, breakout_threshold: float = 1.0):
        self.symbol = symbol
    ...
    def generate_signal(self, price_series: pd.Series) -> int:
        ...
        if self.position == 0:
            if current_price > max_price * (1 + self.breakout_threshold / 100):
                self.position = 1
                self.last_signal = 'long'
                return 1  # Long entry
            elif current_price < min_price * (1 - self.breakout_threshold / 100):
                self.position = -1
    ...
    def backtest(self, price_series: pd.Series) -> Dict[str, Any]:
        """
        Backtest the breakout strategy on a price series.
