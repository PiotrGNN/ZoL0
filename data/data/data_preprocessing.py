...
def clean_data(df: pd.DataFrame, fill_method: str = 'median') -> pd.DataFrame:
    ...
    if fill_method == 'median':
        ...
        logging.warning("Nieznana metoda uzupełniania braków: %s. Domyślnie użyto mediany.", fill_method)
        fill_value = df_clean.median(numeric_only=True)
        df_clean = df_clean.fillna(fill_value)
    return df_clean
...
def detect_outliers(df: pd.DataFrame, column: str, threshold: float = 3.0) -> pd.DataFrame:
    ...
    try:
        ...
        logging.info("Wykryto %d outlierów w kolumnie %s przy progu z-score %.2f.", num_outliers, column, threshold)
        return df_out
    except Exception as e:
...
def winsorize_series(series: pd.Series, limits: tuple = (0.05, 0.05)) -> pd.Series:
    ...
    try:
        ...
        logging.info("Winsoryzacja danych zakończona. Dolny limit: %.2f, Górny limit: %.2f.", lower_limit, upper_limit)
        return winsorized
    except Exception as e:
        logging.error("Błąd przy winsoryzacji serii: %s", e)
        raise

def preprocess_pipeline(df: pd.DataFrame, price_col: str = 'close', fill_method: str = 'median',
                        outlier_threshold: float = 3.0, winsorize_limits: tuple = (0.05, 0.05)) -> pd.DataFrame:
    """
...
def unit_test_preprocessing():
    ...
    try:
        ...
        df_processed = preprocess_pipeline(df, price_col='close', fill_method='median',
                                           outlier_threshold=3.0, winsorize_limits=(0.05, 0.05))
