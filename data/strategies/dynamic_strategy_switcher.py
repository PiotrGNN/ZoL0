...
class DynamicStrategySwitcher:
    def __init__(self, cooldown_period: int = 300, hysteresis_threshold: float = 0.05):
        ...
        logging.info("DynamicStrategySwitcher zainicjalizowany z cooldown: %d s, hysteresis: %.2f",
                     self.cooldown_period, self.hysteresis_threshold)
    ...
    def analyze_market_conditions(self, market_data: pd.DataFrame, sentiment: dict) -> dict:
        ...
        price_change = (recent_period['close'].iloc[-1] - recent_period['close'].iloc[0]) / recent_period['close'].iloc[0]
    ...
    def decide_strategy(self, market_conditions: dict) -> str:
        ...
        logging.info("Decyzja strategii: %s (trend: %.4f, volatility: %.4f, sentiment: %.4f)",
                     chosen_strategy, trend, volatility, sentiment)
    ...
    def switch_strategy(self, market_data: pd.DataFrame, sentiment: dict) -> str:
        ...
        if current_time - self.last_switch_time < self.cooldown_period:
            logging.info("Cooldown active. Strategia pozostaje bez zmian: %s", self.current_strategy)
...
def unit_test_dynamic_strategy_switcher():
    ...
    try:
        ...
        assert strategy1 in ["trend_following", "mean_reversion"], "Nieprawidłowa strategia."
