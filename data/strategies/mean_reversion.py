...
def generate_mean_reversion_signal(df: pd.DataFrame, window: int = 20, zscore_threshold: float = 1.5,
                                   volume_filter: float = None) -> pd.Series:
    """
    ...
    bands = compute_bollinger_bands(close, window)
    ...
    logging.info("Wygenerowano sygnał mean reversion na podstawie z-score (próg: %.2f).", zscore_threshold)
    return signal
