...
def calculate_support_resistance(df: pd.DataFrame, window: int = 20) -> (float, float):
    ...
    logging.info(
        "Obliczono wsparcie: %.2f i opór: %.2f dla okna %d.",
        support,
        resistance,
        window,
    )
...
def breakout_signal(df: pd.DataFrame, support: float, resistance: float, volume_threshold: float = 1.0) -> int:
    ...
    if close > resistance and volume > avg_volume * volume_threshold:
        logging.info(
            "Sygnał BREAKOUT BUY wygenerowany: cena %.2f > opór %.2f, wolumen %.2f.",
            close,
            resistance,
            volume,
        )
    ...
    # Sygnał sprzedaży, jeśli cena spada poniżej wsparcia
        ...
        logging.info(
            "Sygnał BREAKOUT SELL wygenerowany: cena %.2f < wsparcie %.2f, wolumen %.2f.",
            close,
            support,
            volume,
        )
        ...
        logging.info(
            "Brak sygnału breakout: cena %.2f, wsparcie %.2f, opór %.2f, wolumen %.2f.",
            close,
            support,
            resistance,
            volume,
...
def calibrate_box_range(df: pd.DataFrame, window: int = 20) -> (float, float):
    ...
    logging.info(
        "Skalibrowano box range: wsparcie %.2f, opór %.2f.",
        calibrated_support,
