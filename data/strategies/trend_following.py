...
def generate_trend_following_signal(df: pd.DataFrame, adx_threshold: float = 25, macd_threshold: float = 0,
                                    channel_window: int = 20, liquidity_threshold: float = 1000) -> pd.Series:
    """
    ...
    if avg_volume < liquidity_threshold:
        logging.info("Brak wystarczającej płynności (średni wolumen: %.2f). Sygnały nie będą generowane.", avg_volume)
...
def unit_test_trend_following():
    ...
    try:
        ...
        signal = generate_trend_following_signal(df, adx_threshold=25, macd_threshold=0, channel_window=10, liquidity_threshold=1000)
