...
class ExchangeConnector:
    ...
    def __init__(self, exchange: str, api_key: str, api_secret: str, base_url: str = None):
        ...
        self.session.headers.update({"X-MBX-APIKEY": self.api_key})  # Domyślny nagłówek dla Binance; inne giełdy mogą wymagać innej konfiguracji
    ...
    def _request(self, method: str, endpoint: str, params: dict = None, signed: bool = False) -> dict:
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                logging.info("Zapytanie %s %s z parametrami %s zakończone sukcesem.", method, endpoint, masked_params)
                return data
            except requests.exceptions.RequestException as e:
                logging.warning("Błąd zapytania (próba %d/%d): %s", attempt, retries, e)
                if attempt == retries:
                    logging.error("Przekroczono maksymalną liczbę prób dla zapytania: %s", endpoint)
                    raise
                time.sleep(2)
