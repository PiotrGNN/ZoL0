...
def monte_carlo_var(returns: pd.Series, num_simulations: int = 1000, horizon: int = 1, confidence_level: float = 0.95) -> float:
    """
...
def recommend_rebalancing(hh_index: float, threshold: float = 0.2) -> str:
    ...
    if hh_index > threshold:
        ...
        recommendation = "Hold"
    logging.info("Rekomendacja rebalansowania: %s (HH index: %.4f, threshold: %.4f)", recommendation, hh_index, threshold)
    return recommendation

def assess_portfolio_risk(returns: pd.DataFrame, rolling_window: int = 30, confidence_level: float = 0.95) -> pd.DataFrame:
    """
    ...
    """
    var_series = returns.rolling(window=rolling_window).apply(lambda x: calculate_var(pd.Series(x), confidence_level), raw=False)
    cvar_series = returns.rolling(window=rolling_window).apply(lambda x: calculate_cvar(pd.Series(x), confidence_level), raw=False)
