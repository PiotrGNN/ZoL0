    ...
    try:
        ...
        def market_data_generator():
            while True:
                ...
                yield {"volatility": np.random.uniform(0.005, 0.03), "volume": np.random.uniform(1000, 5000)}
        
        def performance_generator():
            while True:
                # Symulacja: losowy drawdown i profit
                yield {"drawdown": np.random.uniform(0, 0.15), "profit": np.random.uniform(-100, 100)}
        
        optimizer = RealTimeOptimizer()
        # Uruchomienie monitoringu w osobnym wątku
        monitor_thread = threading.Thread(target=optimizer.monitor_and_optimize, args=(market_data_generator(), performance_generator(), 10))
