...
from data.data.data_preprocessing import clean_data, compute_log_returns, detect_outliers, winsorize_series, preprocess_pipeline
...
class TestDataProcessing(unittest.TestCase):
    ...
    def test_detect_outliers(self):
        ...
        self.assertIn("is_outlier", df_out.columns, "DataFrame powinien zawierać kolumnę 'is_outlier'.")
    ...
    def test_winsorize_series(self):
        ...
        self.assertTrue((winsorized >= series.quantile(0.05)).all() and (winsorized <= series.quantile(0.95)).all(),
                        "Wartości winsoryzowane powinny być w określonych granicach.")

    def test_preprocess_pipeline(self):
        # Test kompleksowego pipeline'u przetwarzania danych
        df_processed = preprocess_pipeline(self.df, price_col="close", fill_method="median",
                                           outlier_threshold=2.5, winsorize_limits=(0.05, 0.05))
        self.assertIn("log_return", df_processed.columns, "Przetworzone dane powinny zawierać kolumnę 'log_return'.")
...
class TestDataStorageAndHistoricalData(unittest.TestCase):
    ...
    def test_update_csv(self):
        ...
        self.assertIn("2023-01-11", df_updated["timestamp"].astype(str).iloc[-1], "Nowy rekord nie został dodany.")
    ...
    def test_load_from_db(self):
        ...
        self.assertGreaterEqual(len(records), 1, "Tabela w bazie SQLite powinna zawierać co najmniej jeden rekord.")
