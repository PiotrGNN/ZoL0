...
def kelly_criterion_position_size(win_rate: float, win_loss_ratio: float, capital: float) -> float:
    ...
    logging.info("Kelly criterion position size obliczone: %.4f (Kelly fraction: %.4f)", position_size, kelly_fraction)
    return position_size
...
def dynamic_position_size(capital: float, risk_per_trade: float, stop_loss_distance: float, market_volatility: float) -> float:
    """
    ...
    logging.info("Dynamic position size obliczone: %.4f (base: %.4f, adjustment: %.4f)", dynamic_size, base_size, adjustment_factor)
    return dynamic_size
