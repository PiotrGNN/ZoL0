...
class SentimentAnalyzer:
    ...
    def analyze_text(self, text: str) -> Dict[str, float]:
        ...
        try:
            ...
            if result and isinstance(result, list) and "label" in result[0]:
                ...
                logging.info("Analiza tekstu zakończona: %s -> %s (score: %.4f)", text, mapped_label, score)
    ...
    def weighted_sentiment(self, texts_with_source: List[Tuple[str, str]], source_weights: Dict[str, float]) -> Dict[str, float]:
        """
...
def unit_test_sentiment_analysis():
    ...
    try:
        ...
        for res in results:
            assert any(label in res for label in ["NEGATIVE", "NEUTRAL", "POSITIVE"]), "Brak poprawnej etykiety w wyniku."
