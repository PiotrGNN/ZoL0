...
sentiment_ai.py
--------------
Moduł do analizy sentymentu rynku na podstawie różnych źródeł danych.
"""
...
class SentimentAnalyzer:
    ...
    def _analyze_source(self, source: str) -> float:
        ...
        """
        # Symulacja analizy sentymentu z różnych źródeł
        if source == "twitter":
            return random.uniform(-0.8, 0.8)  # Twitter ma większy rozrzut
        elif source == "news":
            return random.uniform(-0.6, 0.6)  # Wiadomości są bardziej neutralne
        elif source == "forum":
            return random.uniform(-0.9, 0.9)  # Fora mają skrajne opinie
        elif source == "reddit":
            return random.uniform(-0.7, 0.7)  # Reddit jest pomiędzy
        else:
            return random.uniform(-0.5, 0.5)  # Domyślna wartość
