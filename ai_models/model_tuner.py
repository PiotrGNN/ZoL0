...
class ModelTuner:
    def __init__(self, model_class, param_space, metric='mse', n_trials=50, cv_splits=5, random_state=42):
        """
    ...
    def objective(self, trial):
        ...
        logging.info("Próba %d: hiperparametry: %s, średni %s: %.4f", trial.number, params, self.metric, mean_score)
        return mean_score
    ...
    def tune(self, X, y):
        ...
        logging.info("Tuning zakończony. Najlepsze hiperparametry: %s, Najlepszy wynik (%s): %.4f",
                     self.best_params, self.metric, self.best_score)
    ...
    def report(self):
        ...
        for trial in self.study.trials:
            logging.info("Próba %d: hiperparametry: %s, wynik: %.4f", trial.number, trial.params, trial.value)
...
if __name__ == "__main__":
    try:
        ...
        tuner = ModelTuner(model_class=RandomForestRegressor,
